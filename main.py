...
class PIDState:
    ...
    def __init__(self, sensorTemp, targetTemp, targetHeat=5, kp=100.0, ki=1., kd=40.):
...
async def PIDControl(PID, sensor, interval):
    while True:
        PID.e2 = PID.e1
        PID.e1 = PID.e
        # Read temperature.
        PID.temp = sensor.temperature
        # Print the value.
...
async def logwrite(logger, coordinator, interval, serial=True, flash=False):
    while True:
        while (coordinator.buffer_out_bytes_free > 0) and (len(logger.readings['u']) > 0):
...
class Coordinator:
    ...
    def __init__(self, serial, buf_in_size=256, buf_out_size=256):
        ...
        self.serial.write_timeout = 0.08
...
async def main():
    thisPID = PIDState(sensor.temperature, targetTemp, 2.5, 100., 1., 40.)
    thisLogger = Logger(time.monotonic())
    thisCoordinator = Coordinator(serial)

    PID_task = asyncio.create_task(PIDControl(thisPID, sensor, 0.3))
    logread_task = asyncio.create_task(logread(thisPID, thisLogger, 0.1))
    logwrite_task = asyncio.create_task(logwrite(thisLogger,thisCoordinator, 0.2, serial=True, flash=True))
    serial_read_task = asyncio.create_task(readSerial(thisCoordinator, 0.25))
    serial_write_task = asyncio.create_task(writeSerial(thisCoordinator, 0.25))
    msg_read_task = asyncio.create_task(readMessages(thisCoordinator, thisPID, 0.6))
